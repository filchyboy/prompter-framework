...
path.join(__dirname, '../templates/templates.js');
path.join(__dirname, '../templates/taskTypes.js');

// Dynamic imports for ES modules
let templates$2, taskTypes$2;

const initializeData = async () => {
  const templatesModule = await Promise.resolve().then(function () { return templates$1; });
  const taskTypesModule = await Promise.resolve().then(function () { return taskTypes$1; });
  
  templates$2 = templatesModule.templates;
  taskTypes$2 = taskTypesModule.taskTypes;
};

const generatePrompt = (formData, taskType) => {
  if (!templates$2 || !taskTypes$2) {
    throw new Error('Data not initialized. Call initializeData() first.');
  }
  
  const template = templates$2[taskType];
  if (!template) {
...
const validateTaskType = (taskType) => {
  ...
  return Object.keys(taskTypes$2).includes(taskType);
};
...
const getAvailableTaskTypes = () => {
  ...
  return Object.keys(taskTypes$2);
};
...
var taskTypes$1 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  taskTypes: taskTypes
});

export { generatePrompt, getAvailableTaskTypes, initializeData, savePromptToFile, validateTaskType };
